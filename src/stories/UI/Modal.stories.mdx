...
Modal is used and rendered internally by [DialogTrigger](?path=/docs/overlays-dialogtrigger--a-basic-dialog-trigger); so check that out if you haven't already, some of the api is exposed which is described more below.
...
@st-import Modal, [
  ...
  exitDone,
  content,
  backdrop] from "@actionishope/shelley/components/Modal/modal.st.css/modal.st.css";
@st-scope MyTheme {
