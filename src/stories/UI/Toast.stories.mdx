...
Toasts are displayed in a priority queue. The default priority is 0. The higher the priority, the more important the toast is. The priority queue is managed by the `<ToastProvider/>` component. The default limit is 5 toasts. When the limit is reached, additional toasts are queued until the user dismisses one.

#### Priority 0 - Info (default)
...
<Source
  ...
  code={dedent`
    <Button
      onClick={() => state.add({ title: "New toast" }, { priority: 1 })}
    >
      Add toast!
    </Button>
  `}
...
<Source
  ...
  code={dedent`
    <Button
      onClick={() => state.add({ title: "New toast" }, { priority: 2 })}
    >
      Add toast!
    </Button>
  `}
...
<Source
  ...
  code={dedent`
    <Button
      onClick={() => state.add({ title: "New toast" }, { priority: 3 })}
    >
      Add toast!
    </Button>
  `}
...
Toasts can have actions. Actions are rendered as buttons. When an action is clicked, the toast is dismissed ( if `shouldCloseOnAction` is `true`) and the action callback is executed.
          ...
          state.add(
            {
              ...
              action: {
                actionLabel: "Fix",
                onAction: () => console.log("Let's fix it!"),
              },
            },
            { priority: 3 }
          )
