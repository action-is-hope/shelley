...
```ts
import { Menu } from "@actionishope/shelley";
import { Item } from "@react-stately/collections";
...
<Preview>
  <Story name="a) Menu">
    <StoryCenteredInline>
      <Menu onAction={(key) => console.log(key)} aria-label="Dynamic menu">
        <Item key="publish">Publish</Item>
        <Item key="archive">Archive</Item>
        <Item key="delete">Delete</Item>
      </Menu>
...
See also Adobes React component, it's similar! https://react-spectrum.adobe.com/react-spectrum/Menu.html
