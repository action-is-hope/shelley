...
<Source
  language="tsx"
  code={dedent`
    import { DisclosureGroup } from "@actionishope/shelley/Disclosure";
  `}
/>
...
custom hook. It initializes the state variables for `isOpen` (whether the disclosure item
...
is set dynamically based on the `isOpen` state.
