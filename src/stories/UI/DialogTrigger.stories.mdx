...
Modal and Popup us react-focus-on internally and you have access to that api via `focusOnProps`. Some options are more relavant than others depending on the type. Setting `scrollLock` to false is more useful on a Popup for instance; `shards` can be useful if working with partial screen takeovers and such.
